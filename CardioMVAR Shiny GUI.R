...
ui <- fluidPage(
  ...
  wellPanel(
    ...
    h3("VAR model order:"),
    tags$hr(),
    checkboxInput("Use_order", "Use customed model order (otherwise the best model order
                  according to the AIC will be used)", value = FALSE),
    numericInput("order", "Customed Model Order", value = 16, min = 1, max = 30, width = 70, step = 1), 
    br(),
    h3("Multiple comparisons settings:"),
    tags$hr(),
    selectInput("mc_correction", "Select a correction method for multiple comparisons", 
                choices = rev(p.adjust.methods), "bonferroni"),
    br(),
    ...
    tags$hr(),
    checkboxInput("Use_weight", "Weight estimates using a Gaussian function", value = FALSE),
    br(),
    h3("Plot settings:"),
    tags$hr(),
  ...
  wellPanel(
    ...
    fluidRow(
      column(4),
      column(4,  actionButton("calculate", "Calculate Results")),
      column(4),
    )
  ),
  wellPanel(
    ...
    fluidRow(
      column(4),
      column(4, downloadButton("download", "Download Calculated Results")),
      column(4),
    ),
    br(),
    fluidRow(
      column(4),
      column(4, downloadButton("download2", "Download Model Structure")),
      column(4),
    )
  )),
  column(4,
  wellPanel( 
    ...
    br(),
    p("Specify the path that you want to isolate from the closed-loop model by selecting the specific input and output
    ...
    br(),
    p("Transfer functions will then be calculated from the selected input to the chosen output, and will reflect the variability
  ...
  wellPanel(
    ...
    br(),
    p("Color legend: closed-loop estimates from model without modification for immediate effects (black),
  ...
  fluidRow(
    column(12,
           wellPanel(
             ...
             p("Each closed-loop model is estimated by calculating a Vector Autoregressive (VAR) model from the data. Additionally, the
               ...
               users choose the appropriate immediate transfer path they want to include in the analyses.",style="text-align:justify;color:black;background-color:lavender;padding:15px;border-radius:10px"),
             br(),
             p("This software uses certain tests for model validation according to the strategy proposed by Seth (4,5) and Ding et al. (6), which was used in his
               ...
               wavelet-detrending can be learned from the work of Li et al. (7), which also discusses other detrending methods.",style="text-align:justify;color:black;background-color:lavender;padding:15px;border-radius:10px"),
             br(),
             tags$hr(),
             h3("Baroreflex sensitivity estimation"),
             br(),
             p("Once the VAR model for a recording has been estimated and validated, the baroreflex sensitivity is calculated as a 
               ...
               of each band.",style="text-align:justify;color:black;background-color:lavender;padding:15px;border-radius:10px"),
             br(),
             tags$hr(),
             h3("Causal coherence"),
             br(),
             p("Causal coherence, as introduced by Faes et al. (10, 11) can be calculated using this software. The software uses the modified
               ...
               value when one of the interactions present at the model is completely supressed.",style="text-align:justify;color:black;background-color:lavender;padding:15px;border-radius:10px"),
           )
    )
  ...
  fluidRow(
    column(12,
           wellPanel(
             h2("References"),
             tags$hr(),
             h5("1. Korhonen I, Mainardi L, Baselli G, Bianchi A, Loula P, 
             ...
             br(),
             h5("2. Faes L, Nollo G. Multivariate Frequency Domain Analysis of Causal Interactions
             in Physiological Time Series. In Biomedical Engineering, Trends in Electronics,
                Communications and Software. 2011"),
             br(),
             h5("3.Hytti H, Takalo R, Ihalainen H. Tutorial on Multivariate Autoregressive Modelling.
              J Clin Monit Comput. 2006;20(1):101-8"),
             br(),
             h5("4. Seth AK. A MATLAB toolbox for Granger causal connectivity analysis. J 
              Neurosci Methods. 2010;186(2):262-73"),
             br(),
             h5("5. Seth AK. Granger Causal Conectivity Analysis: A MATLAB Toolbox. 2011."),
             br(),
             h5("6. Ding M, Bressler S, Yang W, Liang H. Short-window spectral analysis of cortical 
             ...
             br(),
             h5("7. Li L, Liu C, Li K, Liu C. Comparison of detrending methods in spectral analysis of 
             ...
             br(),
             h5("8. Barbieri R, Parati G, Saul JP. Closed- versus Open-Loop Assessment of Heart Rate
             Baroreflex. IEEE Eng Med Biol Mag. 2001;20(2):33-42."),
             br(),
             h5("9. McLoone V, Ringwood JV. A system identification approach to baroreflex sensitivity
              estimation."),
             br(),
             h5("10. Faes L, Porta A, Antolini R, Nollo G. Role of causality in the evaluation of coherence and 
                transfer function between heart period and systolic pressure in humans. Comput Cardiol.
                2004;277-80."),
             br(),
             h5("11. Faes L, Mase M, Nollo G, Chon KH, Florian JP. Measuring postural-related changes of spontaneous
...
server <- function(input, output, session) {
  ...
  Data$Structure <- list()
  Data$Validity <- FALSE
  output$text_timesegment <- renderText({return("Time segment: no time segment has been chosen")})
  output$text_stationary <- renderText({return("Stationarity:")})
  output$text_model <- renderText({return("Model according to AIC:")})
  output$text_model2 <- renderText({return("Chosen model:")})
  output$text_stability <- renderText({return("Stability:")})
  output$text_whitenoise <- renderText({return("Residuals:")})
  output$text_validity <- renderText({return("Model Evaluation:")})
  ...
  observeEvent(input$upload, {
    ...
    RR <- data$RR
    N <- NROW(Time)
    saveData <- list(Time = Time, RR = RR, SBP = SBP)
    ...
    output$Raw <- renderPlot({
      ggplot2::ggplot(data = data.frame(saveData), ggplot2::aes(Time)) +
        ...
        ggplot2::scale_y_continuous(name = "bpm", sec.axis = ggplot2::sec_axis(trans =~.*1, name = "mmHg")) 
    })
    output$data_file <- renderUI({
      fileInput("data_file", "Upload data file",
                multiple = FALSE, accept = c("text/csv",
  ...
  output$hr_stats <- renderText({
    ...
    if(check_brush){
      ...
      HR <- 60000/RR
      text <- paste("Heart Rate: ", round(mean(HR),4), " bpm (SD ", round(sd(HR),4), " bpm)", sep = "")
      return(text)
    }
  })
  output$sbp_stats <- renderText({
    ...
    if(check_brush){
      ...
      text <- paste("Systolic Blood Pressure: ", round(mean(SBP),4), " mmHg (SD ", round(sd(SBP),4), " mmHg)", sep = "")
      return(text)
    }
  })
  ...
  observeEvent(input$brush_raw,
               {
                ...
                output$text_timesegment <- renderText({return(timesegment)})
                select_time <- (data$Time >= input$brush_raw$xmin & data$Time <= input$brush_raw$xmax)
                select_time <- c(1:NROW(data$Time))[select_time]
                if(NROW(select_time) < 64){
                  message <- "Segment is too short. Cannot be evaluated"
                  output$text_stationary <- renderText({return(paste("Stationarity:", message))})
                  output$text_model <- renderText({return(paste("Model according to AIC:", message))})
                  output$text_model2 <- renderText({return(paste("Chosen model:", message))})
                  output$text_stability <- renderText({return(paste("Stability:", message))})
                  output$text_whitenoise <- renderText({return(paste("Residuals:", message))})
                  output$text_validity <- renderText({return(paste("Model Evaluation:", message))})
                  Data$Validity <- FALSE
                } else{
                  RR <- DetrendByCutoff(data$RR[select_time],  f = freq)
                  SBP <- DetrendByCutoff(data$SBP[select_time],  f = freq)
                  ...
                  model1 <- EstimateVAR(data, warnings = FALSE, 
                                        correction = input$mc_correction)
                  if(input$Use_order){
                    model2 <- EstimateVAR(data, p = input$order, pmax = NULL, warnings = FALSE, 
                                          correction = input$mc_correction)
                  } else {
                    model2 <- model1
                  }
                  stable = ifelse(stationarity, DiagnoseStability(model2), FALSE)
                  whitenoise <- ifelse(stationarity, DiagnoseResiduals(model2,
                                                                       correction = input$mc_correction),
                                       FALSE)
                  ...
                  output$text_validity <- renderText({
                    validity <- ifelse(validity, "Model Evaluation: VALID", "Model Evaluation: NOT VALID")
                    return(validity)
  ...
  observeEvent(input$calculate, {
    ...
    if(check_brush & data_validity){
      ...
      form$"Gaussian Weight" <- ifelse(input$Use_weight, "Gaussian weighted-average was used.", 
      ...
      estimations1 <- GetExpectedValues(freq_model1, str = FALSE, coherence = coherence, 
                                        thr = input$coh_thr, use.coh = input$Use_coh, weight = input$Use_weight)
      ...
      est_form1 <- GetExpectedValues(freq_model2, str = FALSE, coherence = coherence,
                                        thr = input$coh_thr, use.coh = FALSE, weight  = input$Use_weight)
      est_form2 <- GetExpectedValues(freq_model2, str = FALSE, coherence = coherence,
                                        thr = input$coh_thr, use.coh = TRUE, weight = input$Use_weight)
      ...
      output$NDiagram <- renderImage({
        ...
        if(out_var == rr_pos) PlotNoiseDiagram(1, immediate = match(input$path_origin, Names))
        if(out_var == sbp_pos) PlotNoiseDiagram(2, immediate = match(input$path_origin, Names))
        dev.off()
      ...
      output$yesDEL <- renderPlot({PlotTransferFun(freq_model2, out_var, in_var,
                                                   plot.phase = input$Plot_phase)})
      output$open <- renderPlot({PlotTransferFun(freq_model1, out_var, in_var , open = TRUE,
                                                 plot.phase = input$Plot_phase)})
      ...
      output$LFpeak2 <- renderText({
        ...
        LF2 <- paste("Estimated LF modulation peak at", round(LF,4), "Hz ")
        return(LF2)
      })
      ...
      output$barplot3 <- renderPlot({
        barplot((diag(ncol(freq_model2$a0)) - freq_model2$a0)[out_var, in_var],  width = 0.3, xlim = c(-0.2,0.7), col = "red")
      })
      form$lag0 <- round((diag(ncol(freq_model2$a0)) - freq_model2$a0)[out_var, in_var],3)
      ...
      output$noise1_noA0 <- renderPlot({PlotNoiseTransferFun(freq_model1, in_var, in_var,
                                                             plot.phase = input$Plot_phase)})
      output$noise2_noA0 <- renderPlot({PlotNoiseTransferFun(freq_model1, out_var, in_var,
                                                             plot.phase = input$Plot_phase)})
      output$noise1_A0 <- renderPlot({PlotNoiseTransferFun(freq_model2, in_var, in_var,
                                                           plot.phase = input$Plot_phase)})
      output$noise2_A0 <- renderPlot({PlotNoiseTransferFun(freq_model2, out_var, in_var,
                                                           plot.phase = input$Plot_phase)})
      nconRR1 <- NoiseContribution(freq_model2, rr_pos, rr_pos, coherence = coherence, 
                                   thr = input$coh_thr, use.coh = input$Use_coh)
      ...
      output$causal <- renderPlot({PlotCausalCoherence(freq_model2, CCoh)})
      output$causal2 <- renderPlot({PlotCausality(freq_model2, in_var)})
      ...
      Data$Freq_model <- freq_model2
        S <- SimulateWithModel(freq_model2, c(freq_model2$Noise_Spectra[sbp_pos,sbp_pos], freq_model2$Noise_Spectra[rr_pos,rr_pos]), 
                               freq_model2$a0[out_var, in_var], f = freq)
      ...
      RR <- data[,"RR"]
      SBP <- data[, "SBP"]
      output$ar1 <- renderPlot({PSD(RR, f = freq, p = model$p)})
      output$ar2 <- renderPlot({PSD(SBP, f = freq, p = model$p, unit = "mmHg2/Hz")})
      RR_PSD <- PSD(RR, f = freq, p = model$p, plot = FALSE, output = TRUE)
      SBP_PSD <- PSD(SBP, f = freq, p = model$p, plot = FALSE, output = TRUE)
      RR_PSD2 <- RR_PSD / sum(RR_PSD[1:2]) * 100
      SBP_PSD2 <- SBP_PSD / sum(SBP_PSD[1:2]) * 100
      output$arHF1 <- renderText({ return(paste("HF:", round(RR_PSD[1],3), "ms2")) })
      output$arHF11 <- renderText({ return(paste("HF:", round(RR_PSD2[1],3), "n.u.")) })
      output$arHFpeak1 <- renderText({ return(paste("HF peak at", round(RR_PSD[3],3), "Hz")) })
      output$arLF1 <- renderText({ return(paste("LF:", round(RR_PSD[2],3), "ms2")) })
      output$arLF11 <- renderText({ return(paste("LF:", round(RR_PSD2[2],3), "n.u.")) })
      output$arLFpeak1 <- renderText({ return(paste("LF peak at", round(RR_PSD[4],3), "Hz")) })
      output$arHF2 <- renderText({ return(paste("HF:", round(SBP_PSD[1],3), "mmHg2")) })
      output$arHF22 <- renderText({ return(paste("HF:", round(SBP_PSD2[1],3), "n.u.")) })
      output$arHFpeak2 <- renderText({ return(paste("HF peak at", round(SBP_PSD[3],3), "Hz")) })
      output$arLF2 <- renderText({ return(paste("LF:", round(SBP_PSD[2],3), "mmHg2")) })
      output$arLF22 <- renderText({ return(paste("LF:", round(SBP_PSD2[2],3), "n.u.")) })
